...
import { Building2, Car, Home, Package, PackagePlus, RefreshCcw, Search, Shield, ShoppingCart, Store } from "lucide-react";
import Link from "next/link";
import { useMemo, useState } from "react";
import { useRouter } from "next/navigation";
import styles from "./stores.module.css";

export default function StoresClient({character,stores,accounts,canRestock,canManageCatalog,loadError=""}:any){
  ...
  return <div className={styles.page}>
    ...
    <section className={styles.hero}>
      <div>
        <span>Community marketplace stores</span>
        <h2>{stores.length} active storefronts</h2>
        <p>Browse government catalogues, general supplies, and stores owned by community businesses.</p>
      </div>
      <div className={styles.heroButtons}>
        {canManageCatalog && (
          <Link href="/stores/admin" className={styles.adminButton}>
            <PackagePlus size={17}/>
            Create / Manage Products
          </Link>
        )}
        <Store className={styles.heroIcon}/>
      </div>
    </section>
