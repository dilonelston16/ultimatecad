...
export async function POST(request:Request){
 ...
 const totalFine=charges.reduce((n:number,c:any)=>n+Number(c.fine_amount||0),0), totalJail=charges.reduce((n:number,c:any)=>n+Number(c.jail_minutes||0),0), totalPoints=charges.reduce((n:number,c:any)=>n+Number(c.points||0),0), bond=charges.reduce((n:number,c:any)=>n+Number(c.bond_amount||0),0);
